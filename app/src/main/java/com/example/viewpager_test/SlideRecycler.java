...
public class SlideRecycler extends RecyclerView {
    ...
    public SlideRecycler(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        layoutManager = new LinearLayoutManager(context);
        setLayoutManager(layoutManager);
        SlideRecycler.context = context;
    ...
    private void move() {
        ...
        if (add != -1) {
            View view = Objects.requireNonNull(findViewHolderForAdapterPosition(add)).itemView;
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        if (add==-1)
            ...
            noEx=false;
        getParent().requestDisallowInterceptTouchEvent(noEx);
         ...
         */
            ...
            if (!isSwipe && !isSroll && Math.abs(movex) > 50) {
                ...
                //向左 在里面
                    ...
                    if (temp != -1) {
                        add = temp;
                        exSize = Objects.requireNonNull(findViewHolderForAdapterPosition(add)).itemView.getWidth() - getWidth();
                        toRight = false;
                        toLeft = true;
                    }
                }
    ...
    private void playBack(int start, final int end) {
        ...
        final View view = Objects.requireNonNull(findViewHolderForAdapterPosition(add)).itemView;
    ...
    private int getPosition(MotionEvent e) {
        ...
        for (int i = f; i <= l; i++) {
            View view = Objects.requireNonNull(findViewHolderForAdapterPosition(i)).itemView;
            int[] location = new int[2];
            view.getLocationOnScreen(location);
            if (e.getY()+dp2px(topview+10) < location[1] && e.getY()+dp2px(topview+10) > location[1] - view.getHeight() && e.getX() > location[0] && e.getX() < location[0] + view.getWidth()) {
                return i;
            }
        }
        return -1;
